...
export default function SeriesPage() {
  ...
  const [filters, setFilters] = useState({
    ...
    sortBy: searchParams.get('sortBy') || 'firstAirDate',
  ...
  const fetchSeries = async () => {
    ...
    try {
      ...
      console.log('URL générée:', `/api/series?${params.toString()}`);

      const response = await fetch(`/api/series?${params.toString()}`);
      const data = await response.json();
      
      if (data.success) {
        console.log('Séries reçues:', data.data.series.length);
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <div className="container mx-auto px-6 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-white mb-4">Toutes les séries</h1>
          <p className="text-gray-400">
            {pagination ? `${pagination.totalCount} séries disponibles` : 'Chargement...'}
          </p>
        </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="flex flex-col sm:flex-row gap-2">
                ...
                >
                  <option value="firstAirDate">Date de première diffusion</option>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16">
            <h2 className="text-2xl font-bold text-white mb-4">Aucune série trouvée</h2>
