...
export default function Top10Series() {
  ...
  useEffect(() => {
    const fetchTop10Series = async () => {
      try {
        const response = await fetch('/api/series/top10');
        const data = await response.json();
        ...
        console.error('Erreur lors du chargement des top 10 séries:', error);
      } finally {
        setIsLoading(false);
      }
    };
            ...
            return (
              <div key={seriesItem.id} className="flex-shrink-0 w-32 md:w-48 bg-gray-800 rounded-lg overflow-hidden hover:scale-105 transition-transform cursor-pointer group/series relative flex flex-col">
                ...
                <div className="relative h-48 md:h-72 overflow-hidden">
                  {/* Rank Badge - Style amélioré avec # */}
                  <div className="absolute top-2 left-2 z-20 text-lg md:text-2xl font-black w-10 h-10 md:w-12 md:h-12 rounded-lg flex items-center justify-center" style={{
                    color: 'transparent',
                    WebkitTextStroke: '1.5px #3b82f6',
                    textShadow: '0 0 8px rgba(59,130,246,0.9), 0 0 16px rgba(59,130,246,0.6)',
                  ...
                  {seriesItem.rating && seriesItem.rating > 0 && (
                    <div className="absolute top-2 right-2 bg-yellow-500 text-black px-2 py-1 rounded-md text-xs font-bold shadow-lg flex items-center space-x-1 z-30">
                  ...
                  {seriesItem.poster ? (
                    <div className="relative w-full h-full group-hover/series:brightness-75 transition-all duration-300">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover/series:opacity-100 transition-opacity duration-300">
                        <button
                          className="bg-sky-400 rounded-full w-12 h-12 md:w-16 md:h-16 flex items-center justify-center hover:bg-sky-500 transition-colors shadow-lg"
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            // Rediriger vers le premier épisode de la première saison
                            window.location.href = `/watch/series/${seriesItem.id}/1/1`;
                          }}
                        >
